...
import 'package:adv_job_landing/core/utils/constants.dart';
import 'package:adv_job_landing/core/utils/typedefs.dart';
import 'package:adv_job_landing/src/auth/data/models/user_model.dart';
...
class AuthRemoteDataSourceImpl implements AuthRemoteDataSource {
        ...
        case UpdateUserAction.password:
          ...
          await _authClient.currentUser?.reauthenticateWithCredential(
            EmailAuthProvider.credential(
              ...
              password: newData['old password'] as String,
            ),
          );
          await _authClient.currentUser?.updatePassword(
            newData['new password'] as String,
          );
  ...
  Future<void> _setUserData(User user, String fullBackEmail) async {
    await _cloudStoreClient.collection('users').doc(user.uid).set(
      LocalUserModel(
        uid: user.uid,
        email: user.email ?? fullBackEmail,
        points: 0,
  ...
  Future<void> _updateUserData(DataMap data) async {
    await _cloudStoreClient.collection
      ('users').doc(_authClient.currentUser?.uid).update(data);
  }
}
